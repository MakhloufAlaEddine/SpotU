...
- **Slice 06** : 6 fichiers de cadrage créés dans `/app/docs/migration/SLICE_06_*.md` pour `GET /api/users/{user_id}/followers` + `GET /api/users/{user_id}/following`. Auth optionnelle via `get_optional_auth`. Tables : `user_follows` (JOIN source), `users`, `user_blocks` (EXISTS subquery is_blocked). Piège critique : noms de champs asymétriques `is_following_back` (followers) vs `follows_back` (following). Pas de pagination, pas de 404, ORDER BY u.name ASC. 12 cas de test. Niveau de risque : FAIBLE. Généré le 2026-02-XX.
