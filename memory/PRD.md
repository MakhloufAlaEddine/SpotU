...
- **Slice 05** : 6 fichiers de cadrage créés dans `/app/docs/migration/SLICE_05_*.md` pour `POST /api/users/{user_id}/follow` + `DELETE /api/users/{user_id}/follow`. Table `user_follows` PK composite (follower_id, following_id). Asymétrie critique : follow vérifie self (400) et existence cible (404) ; unfollow est entièrement silencieux. Double follow idempotent via `ON CONFLICT DO NOTHING`. `followers_count` recalculé SELECT post-mutation. 12 cas de test. Niveau de risque : FAIBLE. Généré le 2026-02-XX.
