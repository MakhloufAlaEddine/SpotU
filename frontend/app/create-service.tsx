...
export default function CreateServiceScreen() {
  ...
  const goNext = () => {
    ...
    if (step === 4) {
      // Validate all locations have a schedule
      ...
      {locations.map((loc, locIdx) => {
        ...
        return (
          <View key={loc.id} style={s.locSection}>
            ...
            {/* Schedule type cards – same as StepDate in create.tsx */}
  ...
  // ─── Step 5 ───────────────────────────────────────────────────────────────
  const totalSlots = locations.reduce((acc, loc) => {
    if (loc.scheduleType === 'once') return acc + (loc.eventDateTime ? 1 : 0);
    return acc + Object.values(loc.recurringSchedule).reduce((s, a) => s + a.length, 0);
  }, 0);
  ...
  const renderQualityScore = () => {
    const criteria = [
      ...
      { ok: totalSlots > 0, label: 'Au moins un créneau' },
    ];
      ...
      <View style={s.stepContent}>
        ...
        <View style={s.summaryCard}>
          ...
          <View style={s.summaryMeta}>
            <View style={s.summaryMetaItem}><Ionicons name="pricetag-outline" size={14} color={ORANGE} /><Text style={s.summaryMetaText}>{price}€ / séance</Text></View>
            <View style={s.summaryMetaItem}><Ionicons name="time-outline" size={14} color={ORANGE} /><Text style={s.summaryMetaText}>{duration} min</Text></View>
            <View style={s.summaryMetaItem}><Ionicons name="people-outline" size={14} color={ORANGE} /><Text style={s.summaryMetaText}>{maxParticipants} pers.</Text></View>
            {domainObj && <View style={s.summaryMetaItem}><Ionicons name="fitness-outline" size={14} color={ORANGE} /><Text style={s.summaryMetaText}>{domainObj.label_fr}</Text></View>}
          </View>
        </View>
        {locations.length > 0 && (
...
// ─── Schedule styles (matches create.tsx sc object exactly) ──────────────────
...
const s = StyleSheet.create({
  ...
  stepCircle: { width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: Colors.border, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.background },
  ...
  input: { backgroundColor: Colors.card, borderRadius: Radius.md, borderWidth: 1, borderColor: Colors.border, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: Colors.foreground },
  ...
  motiveBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, backgroundColor: ORANGE_LIGHT, borderRadius: Radius.lg, padding: 14, borderWidth: 1, borderColor: ORANGE_BORDER },
  ...
  infoNote: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: ORANGE_LIGHT, borderRadius: Radius.md, padding: Spacing.sm, borderWidth: 1, borderColor: ORANGE_BORDER },
  ...
  // Quality
  qualityCard: { backgroundColor: Colors.card, borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1 },
  qualityBadge: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  ...
  // Summary
  summaryCard: { backgroundColor: Colors.card, borderRadius: Radius.xl, padding: Spacing.md, borderWidth: 1, borderColor: ORANGE_BORDER, gap: 8 },
  ...
  // Bottom nav
  bottomNav: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: Spacing.md, paddingVertical: 12, backgroundColor: Colors.backgroundSecondary, borderTopWidth: 1, borderTopColor: Colors.border },
  prevBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12, borderRadius: Radius.full, borderWidth: 1, borderColor: Colors.border },
  prevBtnText: { fontSize: 14, fontWeight: '600', color: Colors.foreground },
  nextBtn: { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12, borderRadius: Radius.full, backgroundColor: ORANGE },
